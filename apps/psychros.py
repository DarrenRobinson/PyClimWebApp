...
def app(app, epw, ui):
    ...
    MartinezLimit = True
    PlotMonthly = st.sidebar.checkbox(
        "Plot Monthly",
        value=True,
        help="If FALSE then there is no distinction between data points for different months",
    )
    PlotEvapCool = st.sidebar.checkbox(
        "PlotEvapCool", value=True, help="Efficiency of the evaporative cooling process"
    )
    colour = st.sidebar.color_picker(
        "Colour Picker",
        value="#0C791A",
        help="By default when applying filters, all data should be plotted in the same colour",
    )
    # colour = '#0C791A'

    with st.sidebar.form(key="psychros"):
        LLdbt = st.number_input(
            "LLdbt",
            min_value=float(min_temp),
            value=25.0,
            step=0.5,
            help="Temperature above which data points are shifted to mimic evaporative cooling",
        )  # lower limit of temperature: above which data is shifted
        st.form_submit_button(label="Apply Change")

    EvapCoolEff = st.sidebar.slider(
        "EvapCoolEff",
        0.0,
        1.0,
        0.7,
        help="If TRUE then a separate plot is produced with evaporative cooling mimicked",
    )  # Proportion of wbtd thar data s shifted to.
    ...
    if filter_applied:
        for i in range(1, 13):
            daynum_list[i - 1] = len(
                epw.dataframe[epw.dataframe["Month"] == i]["Day"].unique()
            )
        hour_range = (
            (
                st.session_state.psychros_end_hour
                - st.session_state.psychros_start_hour
                + 2
            )
            if (
                st.session_state.psychros_end_hour
                ...
                (24 - st.session_state.psychros_start_hour)
                + (st.session_state.psychros_end_hour - 1)
                + 2
            )
        )
    ...
    if PlotEvapCool:
        ...
        for plotpoints in range(0, len(temp_list)):
            ...
            if MartinezLimit:
                LLdbt = (
                    29 + g_list[plotpoints] / -0.0055
                )  # where -0.0055 = dy/dx of PDEC line
