...
def app(file_name, title, ui_helper, file_list, lat, longitude, timezone):
    ...
    There are two stages to the use of the PyClim Web App. First, a weather file needs to be selected. The is done by navigating down the hierarchy in the Advanced Search option from Region (continents) through country to state (where applicable) and then selecting the weather file from the list of those available. Alternatively, you can also search by ranking of weather files based on their Euclidean distance (distance, as the crow flies) from the coordinates of your targey site location (just enter the latitude and longitude), or you can directly perform a keyword search for the weather data file or city.
