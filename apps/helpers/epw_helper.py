...
class EPWHelper:
    ...
    def _calculate_distance(self, df_latlng, lat_or_lng, default_val):
        user_latlng = (
            st.session_state[lat_or_lng]
            if lat_or_lng in st.session_state
            else default_val
        )
    ...
    def _sort_list_by_distance(self, df):
        dlat, lat2, lat1 = self._calculate_distance(
            df["lat"], "user_lat", self.default_lat
        )
        dlon, lon2, lon1 = self._calculate_distance(
            df["lon"], "user_lng", self.default_lon
        )

        a = ((dlat / 2).apply(math.sin) ** 2) + math.cos(lat1) * (
            lat2.apply(math.cos)
        ...
        temp_df["c"] = 2 * temp_df.apply(
            lambda x: math.atan2(x["a_sq"], x["one_minus_a_sq"]), axis=1
        )
    ...
    def _get_weather_data_file_list(self):
        ...
        weather_data_file_list = weather_data_file_list.rename(
            columns={
                3: "region",
                4: "country",
                5: "state",
        ...
        weather_data_file_list = (
            self._sort_list_by_distance(weather_data_file_list)
            if self._is_sort_by_distance()
            else weather_data_file_list.sort_values(["region", "country", "state"])
        )
    ...
    def advanced_search(self):
        ...
        with expander:
            st.radio(
                "Search",
                [self.sort_list, self.filter_list],
                key="filter_option",
                on_change=self._filter_settings_reset,
            )
            ...
            if "filter_option" in st.session_state:
                if st.session_state.filter_option == self.sort_list:
                    with st.form(key="advanced_search"):
                        st.number_input(
                            "Latitude", -90.0, 90.0, 53.4, 0.1, key="user_lat"
                        )
                        st.number_input(
                            "Longitude", -180.0, 180.0, -1.5, 0.1, key="user_lng"
                        )
                ...
                if st.session_state.filter_option == self.filter_list:
                    ...
                    st.selectbox(
                        ...
                        regions_dropdown,
                        key="region",
                        format_func=lambda x: _format(x, "region"),
                    )

                    countries_filtered = self.weather_data_file_list.drop_duplicates(
                        ["region", "country"]
                    )
                    ...
                    if "region" in st.session_state:
                        if st.session_state["region"] == "all":
                            ...
                            country_dropdown += countries_filtered[
                                countries_filtered["region"]
                    ...
                    st.selectbox(
                        ...
                        country_dropdown,
                        key="country",
                        format_func=lambda x: _format(x, "country"),
                    )

                    states_filtered = self.weather_data_file_list.drop_duplicates(
                        ["country", "state"]
                    ).dropna()
                    ...
                    if "country" in st.session_state:
                        if st.session_state["country"] == "all":
                            ...
                            state_dropdown = []
                        elif not states_filtered[
                            states_filtered["country"] == st.session_state["country"]
                        ]["state"].empty:
                            state_dropdown = ["all"]
                            state_dropdown += states_filtered[
                                states_filtered["country"]
                    ...
                    st.selectbox(
                        ...
                        state_dropdown,
                        key="state",
                        format_func=lambda x: _format(x, "state"),
                    )

        self.weather_data_dropdown = self.weather_data_file_list[
            ["region", "country", "state", "title", "file_url", "lon", "lat"]
        ].to_dict("records")
        ...
        for filter_word in ["state", "country", "region"]:
            if _filter_check(filter_word):
                self.weather_data_dropdown = [
                    v
                    for v in self.weather_data_dropdown
                    if v[filter_word] == st.session_state[filter_word]
                ]
                break
    ...
    def df_filter(self, file_title):
        ...
        direction = (time_var["end_month"] > time_var["start_month"]) | (
            (time_var["end_month"] == time_var["start_month"])
            & (time_var["end_day"] >= time_var["start_day"])
        )
        range = (
            (
                ...
                | (
                    (self.dataframe["Month"] == time_var["start_month"])
                    & (self.dataframe["Day"] >= time_var["start_day"])
                )
            ),
            (
                (self.dataframe["Month"] < time_var["end_month"])
                | (self.dataframe["Month"] == time_var["end_month"])
                & (self.dataframe["Day"] <= time_var["end_day"])
            ),
        )
    ...
    def _df_filter_pipeline(self, start_month_before_end_month, range):
        ...
        #
        filter_operator = (
            operator.__and__ if start_month_before_end_month else operator.__or__
        )
    ...
    def map_viewer(self):
        ...
        view_state = pdk.ViewState(
            latitude=selected["lat"].iloc[0],
            longitude=selected["lon"].iloc[0],
            zoom=4,
            min_zoom=1,
            max_zoom=30,
            height=170,
