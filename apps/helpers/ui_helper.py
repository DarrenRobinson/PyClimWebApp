...
class UIHelper:
    ...
    def base64_to_link_and_graph(self, figure, filename, format, width, height):
        ...
        graph = '<img width={} height={} src="data:image/jpg;base64, {}">'.format(
            width, height, decoded
        )
        href = "<center>Download figure <a href='data:image/{};base64,{}' download='{}'>{}</a></center><br>".format(
            format, decoded, filename + "." + format, format.upper()
        )
        return graph, href
    ...
    def _session_keys_init(self, selected_feature):
        ...
        st.session_state["session_keys"]["start_month"] = (
            selected_feature + "_start_month"
        )
        ...
        st.session_state["session_keys"]["start_hour"] = (
            selected_feature + "_start_hour"
        )
    ...
    def _check_day(self, start_or_end):
        ...
        if (day in st.session_state) & (month in st.session_state):
            ...
            if st.session_state[day] > (
                len(self.days[st.session_state[month]["value"] - 1])
            ):
    ...
    def time_filter(self, feature):
        ...
        start_month_index = (
            st.session_state[start_month]["value"] - 1
            if start_month in st.session_state
            else 0
        )
        end_month_index = (
            st.session_state[end_month]["value"] - 1
            if end_month in st.session_state
            else 11
        )
        ...
        start_day_index = (
            st.session_state[start_day] - 1 if start_day in st.session_state else 0
        )
        end_day_index = (
            st.session_state[end_day] - 1
            if end_day in st.session_state
            else end_days.index(max(end_days))
        )

        # Set the dropdowns to display the previously selected hour if applicable
        start_hour_index = (
            st.session_state[start_hour] - 1 if start_hour in st.session_state else 0
        )
        end_hour_index = (
            st.session_state[end_hour] - 1 if end_hour in st.session_state else 23
        )
        ...
        if end_hour_index >= start_hour_index:
            show_hour = (
                str(start_hour_index + 1) + ":00 to " + str(end_hour_index + 1) + ":00,"
            )
        else:
            show_hour = (
                "1:00 to "
                + str(end_hour_index + 1)
                + ":00 and "
                + str(start_hour_index + 1)
                + ":00 to 24:00,"
            )
