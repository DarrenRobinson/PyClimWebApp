...
from .ClimAnalFunctions import (LumEff, Tground, declin_angle, solar_altitude,
                                sunrise_time, time_diff, timeshift)
...
def app(app, epw, ui, timeshift=timeshift):
    ...
    HDDbase = st.sidebar.slider(
        "Heating Degree-day Base Temperature",
        10.0,
        18.0,
        15.5,
        0.5,
        help="Temperature below which heating is needed",
    )
    CDDbase = st.sidebar.slider(
        "Cooling Degree-day Base Temperature",
        14.0,
        22.0,
        18.0,
        0.5,
        help="Temperature above which cooling is needed",
    )
    ...
    with st.sidebar.form(key="sunWeatherAnalysispath"):
        ...
        Conductivity = st.number_input(
            ...
            3.0,
            1.21,
            help="Soil conductivity (W/(m.K)): ground temperature profile",
        )
        Density = st.number_input(
            ...
            3000,
            1960,
            help="Soil density (kg/m\u00b3): ground temperature profile",
        )
        Cp = st.number_input(
            "Soil Specific Heat Capacity (J/(kg.K))",
            500,
            2000,
            840,
            help="Soil specific heat capacity (J/(kg.K)): ground temperature profile",
        )
    ...
    for i in range(1, 13):
        ...
        for j in range(1, daynum_list[i - 1] + 1):
            ...
            for k in range(1, 25):
                ...
                WindKineticEnergy = (
                    WindKineticEnergy
                    + 0.5 * Rho * winspeed_list[24 * (cumday - 1) + k - 1] ** 3 / 1000
                )
                # Accrue monthly degree days
                # annual mean temp for ground temperature model
                annualmeantemp = annualmeantemp + temp_list[
                    24 * (cumday - 1) + k - 1
                ] / len(temp_list)
                ...
                if solalt > 0 and global_list[24 * (cumday - 1) + k - 1] > 0:
                    ibn = (
                        global_list[24 * (cumday - 1) + k - 1]
                        - diffuse_list[24 * (cumday - 1) + k - 1]
                    ) / math.sin(solalt)
                    if globaleff and diffuse_list[24 * (cumday - 1) + k - 1] > 0:
                        illuminance = global_list[24 * (cumday - 1) + k - 1] * LumEff(
                            globaleff,
                            cumday,
                            solalt,
                            diffuse_list[24 * (cumday - 1) + k - 1],
                        ...
                        illuminance = diffuse_list[24 * (cumday - 1) + k - 1] * LumEff(
                            globaleff,
                            cumday,
                            solalt,
                            diffuse_list[24 * (cumday - 1) + k - 1],
            ...
            if daymeantemp > CDDbase:
                MonthlyCDD_list[i - 1] = MonthlyCDD_list[i - 1] + (
                    daymeantemp - CDDbase
                )
                TotalCDD = TotalCDD + (daymeantemp - CDDbase)
            if daymeantemp < HDDbase:
                MonthlyHDD_list[i - 1] = MonthlyHDD_list[i - 1] + (
                    HDDbase - daymeantemp
                )
    ...
    for i in range(1, 13):
        ...
        for depth in range(0, 21):
            ...
            tground_matrix[i - 1].append(
                Tground(
                    annualmeantemp,
                    amplitude,
                    cum_monthmeandaynum - daynum_list[i - 1] / 2,
                    t_offset,
                    depth,
                    Conductivity,
                    Density,
    ...
    st.write(
        "Annual global horizontal solar irradiation: {0:1.2f}".format(AnnualIgh)
        + ", kWh/m\u00b2"
    )
    st.write("Annual solar diffuse fraction: {0:1.3f}".format(DiffuseFraction))
    st.write(
        "Total annual wind kinetic energy flux: {0:1.2f}".format(WindKineticEnergy)
        + ", kWh/m\u00b2"
    )
    ...
    for month in range(1, 13):
        ...
        ax_ground.plot(
            tground_matrix[month - 1], depth_list, lw=2, c=Colour_list[11 - (month - 1)]
        )
    ...
    ax_temp_hist.hist(
        temp_list, xrange, alpha=0.3, histtype="step", color="darkgray", lw=3
    )
    ...
    ax_wind_hist.hist(
        winspeed_list, xrange, alpha=0.3, histtype="step", color="darkgray", lw=3
    )
    # creates a y2 axis for the cumulative distribution
    ...
    fig_illuminance, ax_illuminance = plt.subplots(
        1, 1, figsize=(12, 6), tight_layout=True
    )
    ...
    graph, href = ui.base64_to_link_and_graph(
        fig_illuminance, fig_title, "jpg", 700, 350
    )
    st.write(graph, href, unsafe_allow_html=True)
    # plots a degree-day histograms

    fig_monthly_degree, ax_monthly_degree = plt.subplots(
        1, 1, figsize=(12, 6), tight_layout=True
    )
    ...
    graph, href = ui.base64_to_link_and_graph(
        fig_monthly_degree, fig_title, "jpg", 700, 350
    )
    st.write(graph, href, unsafe_allow_html=True)
    ...
    Z = Z.reshape(365, 24)
    cp = ax_solar.contourf(
        Y, X, Z, 16, cmap="jet"
    )  #'plasma', 'jet' and 'viridis' are also good cmaps
    fig_solar.colorbar(
        cp, label="Global horizontal solar irradiance, W/m\u00b2"
    )  # Adds a colorbar
    ...
    Z = Z.reshape(365, 24)
    cp = ax_daylight.contourf(
        Y, X, Z, 16, cmap="jet"
    )  #'plasma', 'jet' and 'viridis' are also good cmaps
    if not globaleff:
        fig_daylight.colorbar(
            cp, label="diffuse horizontal illuminance, kLux"
        )  # Adds a colorbar
    else:
        fig_daylight.colorbar(
            cp, label="global horizontal illuminance, kLux"
        )  # Adds a colorbar
