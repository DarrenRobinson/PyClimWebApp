...
def app(app, epw, ui, timeshift=timeshift, groundref=groundref):
    ...
    DiffuseOnly = st.sidebar.checkbox(
        "DiffuseOnly",
        value=False,
        help="If TRUE then only diffuse irradiation is calculated; otherwise direct is also included",
    )
    isotropic = st.sidebar.checkbox(
        "isotropic",
        value=False,
        help="If TRUE then simpler calculations are used for an isotropic sky",
    )
    groundref = st.sidebar.number_input("groundref", 0.0, 1.0, groundref, 0.5)
    timeshift = st.sidebar.slider(
        "Timeshift",
        -0.5,
        0.5,
        timeshift,
        0.5,
        help="This is to handle timing conventions relating to climate data collection",
    )
    ...
    solaz_list = solar_azimuth2(
        day_list_repeat, jhour_x_time, lat, solalt_list, dec_list_repeat
    )
    ...
    if isotropic:
        ...
        ylist = np.linspace(
            0, 90, 10
        )  # Note: theer need to be 19 subdivisions (here and for Z) for 5o bins of altitude
        ...
        Z = Z.reshape(10, 36)
        cp = ax.contourf(
            X, Y, Z, 16, cmap="plasma", alpha=1.0
        ...
        cp = ax.contourf(
            X, Y, Zprime, 16, cmap="plasma", alpha=1.0
        )  # NB: 16 sets number of division; alpha sets opacity; 'magma', 'jet' and 'viridis' are also good cmaps
        fig.colorbar(cp, label="Solar irradiation, MWh/m\u00b2")  # Adds a colorbar
