...
def app(app, epw, ui):
    ...
    for circle in range(90, 0, -10):
        ...
        for orientation in range(0, 360 + AzimuthIncrement, AzimuthIncrement):
            ...
            if circle == 90 and orientation < 360:
                ax_sun.text(
                    95 * math.sin(orientation * pi / 180),
                    95 * math.cos(orientation * pi / 180),
                    str(orientation) + "$^o$",
                    c="darkgray",
        ...
        if circle < 90 and circle != 0:
            ax_sun.text(
                (circle + 1) * math.sin(5 * pi / 180),
                (circle + 1) * math.cos(orientation * pi / 180),
                str(90 - circle) + "$^o$",
                c="darkgray",
                horizontalalignment="center",
                fontsize=8,
    ...
    for month in range(1, 8):
        ...
        if ss < 24:
            ...
            for hour in range(math.ceil(sr), int(sr) + 2 * (12 - int(sr))):
                ...
                azi_list.append(
                    solar_azimuth(day, hour, lat, alt_list[position] * pi / 180, dec)
                )
                sunpath_x.append(
                    (90 - alt_list[position]) * math.sin(azi_list[position])
                )
                sunpath_y.append(
                    (90 - alt_list[position]) * math.cos(azi_list[position])
                )

            alt_list.append(solar_altitude(day, ss, lat, dec) * 180 / pi)
            azi_list.append(
                solar_azimuth(
                    day, ss, lat, solar_altitude(day, ss, lat, dec) * pi / 180, dec
                )
            )
    ...
    if HorizontalProtractor:
        for Theta in range(10, 90, 10):
            ...
            for h_orientation in range(WallAzimuth - 90, WallAzimuth + 100, 10):
                ThetaAdjusted = (
                    math.atan(
                        math.tan(Theta * pi / 180)
                        * math.cos(math.fabs(h_orientation - WallAzimuth) * pi / 180)
                    )
                    * 180
                    / pi
                )
                HorizontalProtractor_x.append(
                    (90 - ThetaAdjusted) * math.sin(h_orientation * pi / 180)
                )
                HorizontalProtractor_y.append(
                    (90 - ThetaAdjusted) * math.cos(h_orientation * pi / 180)
                )
            ax_sun.plot(
                HorizontalProtractor_x,
                HorizontalProtractor_y,
    ...
    fig_title = (
        "Stereographic sunpath diagram, for latitude: "
        + str(int(180 * math.fabs(lat) / pi))
        + "$^o$"
        + str(Hemisphere)
    )
