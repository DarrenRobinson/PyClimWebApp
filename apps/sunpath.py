    ...
    for circle in range(90, 0, -10):
        ...
        for orientation in range(0, 360+AzimuthIncrement, AzimuthIncrement):
            ...
            if circle == 90 and orientation < 360:
                ax_sun.text(95*math.sin(orientation*pi/180),95*math.cos(orientation*pi/180),str(orientation)+ '$^o$', c = 'darkgray', horizontalalignment='center', fontsize=8)
        if circle<90 and circle!=0:
            ax_sun.text((circle+1)*math.sin(5*pi/180),(circle+1)*math.cos(orientation*pi/180),str(90-circle)+ '$^o$', c = 'darkgray', horizontalalignment='center', fontsize=8)
    ...
    for hour in range(0,25):
        ...
        for day in range(1,366):
            ...
            if solar_altitude(summerday,hour,lat,declin_angle(summerday)) > 0:
                ...
                EqT = time_diff(day, EqTonly, 0, 0, 0) if ClockTime == True else 0
                Dec = declin_angle(day)
    ...
    if HorizontalProtractor:
        for Theta in range(10, 90, 10):
            ...
            for h_orientation in range(WallAzimuth-90, WallAzimuth+100, 10):
                ThetaAdjusted = math.atan(math.tan(Theta*pi/180) * math.cos(math.fabs(h_orientation-WallAzimuth)*pi/180)) * 180/pi            
